...
async function instantiate(wasm) {
  let memory = null;
  const writeI128 = (ptr, value) => {
    const bits = BigInt.asUintN(128, value);
    const view = new DataView(memory.buffer);
    view.setBigUint64(ptr, BigInt.asUintN(64, bits), true);
    view.setBigUint64(ptr + 8, BigInt.asUintN(64, bits >> 64n), true);
  };
  const joinI128 = (lo, hi) => (BigInt.asUintN(64, hi) << 64n) | BigInt.asUintN(64, lo);
  const shift = (count) => BigInt(count & 127);
  const imports = { env: {
    joy_bench_now: () => performance.now(),
    fmod: (a, b) => a % b,
    fmodf: (a, b) => Math.fround(Math.fround(a) % Math.fround(b)),
    __lshrti3: (ret, lo, hi, count) => writeI128(ret, joinI128(lo, hi) >> shift(count)),
    __ashlti3: (ret, lo, hi, count) => writeI128(ret, joinI128(lo, hi) << shift(count)),
    __ashrti3: (ret, lo, hi, count) => writeI128(ret, BigInt.asIntN(128, joinI128(lo, hi)) >> shift(count)),
  } };
  ...
  if (typeof wasm !== 'string') {
    return ready(await WebAssembly.instantiate(await WebAssembly.compile(wasm), imports));
  }
  ...
  if (res.headers.get('content-type')?.startsWith('application/wasm')) {
    return ready((await WebAssembly.instantiateStreaming(res, imports)).instance);
  }
  return ready(await WebAssembly.instantiate(await WebAssembly.compile(await res.arrayBuffer()), imports));
}
...
export const browserDom = {
  createElement: (tag) => SVG_TAGS.has(tag)
